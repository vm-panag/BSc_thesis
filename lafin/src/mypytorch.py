...
class SpectralNorm:
    ...
    def compute_weight(self, module: Module, do_power_iteration: bool) -> torch.Tensor:
        ...
        np_wm = weight_mat.cpu().detach().numpy()
        np_v = v.cpu().detach().numpy()
        np_u = u.cpu().detach().numpy()
        ...
        if do_power_iteration:
            with torch.no_grad():
                for _ in range(self.n_power_iterations):
                    ...
                    np_wmt = weight_mat.t().cpu().detach().numpy()
               
                    np_wmt_u = np.matmul(np_wmt, np_u)
                    np_wm_v = np.matmul(np_wm, np_v)

                    wmt_u = torch.from_numpy(np.asarray(np_wmt_u)).to('cuda:0')
                    wm_v = torch.from_numpy(np.asarray(np_wm_v)).to('cuda:0')

                    v = normalize(wmt_u, dim=0, eps=self.eps, out=v)
                    u = normalize(wm_v, dim=0, eps=self.eps, out=u)

                    # v = normalize(torch.matmul(weight_mat.t(), u), dim=0, eps=self.eps, out=v)
        ...
        np_wm_v = np.matmul(np_wm, np_v)
        np_u_wm_v = np.matmul(np_u, np_wm_v)
        u_wm_v = torch.from_numpy(np.asarray(np_u_wm_v)).to('cuda:0')
    ...
    def _solve_v_and_rescale(self, weight_mat, u, target_sigma):
        ...
        return v.mul_(target_sigma / u_wm_v)
        # return v.mul_(target_sigma / torch.matmul(u, torch.matmul(weight_mat, v)))
