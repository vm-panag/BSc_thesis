...
class Lafin():
    def __init__(self, config):
        ...
        # self.inpaint_model.load_generator()

        # input_dummy1 = Variable(torch.zeros(1, 3, 256, 256, dtype=torch.float32).to(config.DEVICE))
        # input_dummy2 = Variable(torch.zeros(1, 1, 256, 256, dtype=torch.float32).to(config.DEVICE))
        # input_dummy3 = Variable(torch.zeros(1, 1, 256, 256, dtype=torch.float32).to(config.DEVICE))

        # print("input_dummy1", input_dummy1.dtype)
        # print("input_dummy1", input_dummy2.dtype)
        # print("input_dummy1", input_dummy3.dtype)

        # torch.onnx.export(self.inpaint_model, 
        # (input_dummy1, input_dummy2, input_dummy3), 
        # "inpaintor.onnx", opset_version=11)
        ...
        if self.config.MODE == 2:
            if self.config.MODEL == 1:
                ...
                print(config.TEST_INPAINT_LANDMARK_FLIST)
